...
import { Search, Plus, Filter, RefreshCw, Swords, Clock, Trophy, Tag, Globe, Pencil, Trash2, Eye, LayoutGrid, List } from 'lucide-react';
...
const kategoriColors: Record<string, string> = {
  ...
  Kewirausahaan: 'bg-amber-100 text-amber-700',
  Sains: 'bg-cyan-100 text-cyan-700',
  'Seni & Budaya': 'bg-purple-100 text-purple-700',
  Olahraga: 'bg-rose-100 text-rose-700',
};

const tingkatColors: Record<string, string> = {
  Kampus: 'bg-slate-100 text-slate-600',
    ...
    <AppLayout activePath="/lomba-management">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Manajemen Lomba</h1>
            <p className="text-slate-500 text-sm mt-1">Kelola daftar kompetisi dan lomba untuk mahasiswa.</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Tutup', value: data.filter(d => d.status === 'closed').length, icon: Clock, color: 'text-orange-600 bg-orange-50' },
            { label: 'Diarsipkan', value: data.filter(d => d.status === 'archived' || d.status === 'expired').length, icon: Globe, color: 'text-slate-600 bg-slate-50' },
          ].map(stat => (
        ...
        <div className="card p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden">
                <button onClick={() => setViewMode('card')} className={`p-2 transition-colors ${viewMode === 'card' ? 'bg-indigo-600 text-white' : 'hover:bg-slate-100 text-slate-500'}`}>
                  <LayoutGrid size={16} />
                </button>
                <button onClick={() => setViewMode('list')} className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-indigo-600 text-white' : 'hover:bg-slate-100 text-slate-500'}`}>
                  <List size={16} />
                </button>
              </div>
            </div>
          ...
          <p className="text-xs text-slate-500">
            Menampilkan <span className="font-semibold text-slate-700">{filtered.length}</span> dari {data.length} lomba
          </p>
        </div>
        ...
        {loading ? (
          ...
          <EmptyState
            icon={<Swords size={40} className="text-slate-300" />}
            title="Belum ada lomba"
            description="Tambahkan lomba pertama untuk mulai mengelola kompetisi mahasiswa."
            action={<button onClick={() => { setEditItem(null); setFormOpen(true); }} className="btn-primary">Tambah Lomba</button>}
          />
        ) : viewMode === 'card' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(lomba => {
              ...
              return (
                <div key={lomba.id} className="card p-5 flex flex-col gap-3 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 group">
                  ...
                  {/* Description */}
                  {lomba.description && (
                    <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">{lomba.description}</p>
                  )}

                  {/* Tags */}
                  <div className="flex flex-wrap gap-1.5">
                    {lomba.kategori && (
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${kColor}`}>
                        <Tag size={9} className="inline mr-1" />{lomba.kategori}
                      </span>
                    )}
                    {lomba.tingkat && (
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${tColor}`}>
                        <Globe size={9} className="inline mr-1" />{lomba.tingkat}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500 border-t border-slate-100 pt-3">
                    {lomba.deadline && (
                      <span className="flex items-center gap-1">
                        ...
                        Deadline: {new Date(lomba.deadline).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </span>
                    )}
                    {lomba.prize && (
                      <span className="flex items-center gap-1 ml-auto">
                        <Trophy size={12} className="text-amber-500" />
                        <span className="truncate max-w-[100px]">{lomba.prize}</span>
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      onClick={() => router.push(`/lomba-management/${lomba.id}`)}
                    ...
                    <button
                      onClick={() => { setEditItem(lomba); setFormOpen(true); }}
                    ...
                    <button
                      onClick={() => setDeleteTarget(lomba.id)}
                  ...
                  return (
                    <tr key={lomba.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-800 text-sm">{lomba.title}</p>
                        <p className="text-xs text-slate-500">{lomba.organizer}</p>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${kategoriColors[lomba.kategori || ''] || 'bg-slate-100 text-slate-600'}`}>
                          {lomba.kategori || '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${tingkatColors[lomba.tingkat || ''] || 'bg-slate-100 text-slate-600'}`}>
                          {lomba.tingkat || '—'}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => router.push(`/lomba-management/${lomba.id}`)} className="p-1.5 rounded-lg hover:bg-indigo-50 text-indigo-600 transition-colors" title="Detail">
                            <Eye size={14} />
                          </button>
                          <button onClick={() => { setEditItem(lomba); setFormOpen(true); }} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors" title="Edit">
                            <Pencil size={14} />
                          </button>
                          <button onClick={() => setDeleteTarget(lomba.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-colors" title="Hapus">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
