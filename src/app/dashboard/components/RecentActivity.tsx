...
import { Trophy, UserPlus, CheckCircle, XCircle, Swords, CalendarDays, Loader2, RefreshCw } from 'lucide-react';
...
export default function RecentActivity() {
  ...
  const fetchActivitiesData = useCallback(async () => {
    ...
    // Map Achievements
    (achievements as AchievementData[] | null)?.forEach(a => {
      const userName = a.users?.name || 'Mahasiswa';
      if (a.status === 'pending') {
        allActivities.push({
          id: `ach-${a.id}`,
          type: 'new_prestasi',
  ...
  return (
    <div className="card overflow-hidden h-full">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        ...
        <button onClick={handleRefresh} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 transition-colors">
